...
import { verificarCodigoDesignerAction, getPendingEmailDesigner } from "../lib/designer.auth.server";

export const loader = async ({ request }: LoaderFunctionArgs) => {
  const emailMascarado = await getPendingEmailDesigner(request);
  if (!emailMascarado) throw redirect("/designer/login");
  return { emailMascarado };
};

export const action = async ({ request }: ActionFunctionArgs) => {
  return verificarCodigoDesignerAction(request);
};

export default function DesignerVerificar() {
    ...
    <div style={{ minHeight: "100vh", background: "#111", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Inter, system-ui, sans-serif" }}>
      <div style={{ background: "#fff", borderRadius: "20px", padding: "48px 40px", width: "100%", maxWidth: "380px" }}>
        ...
        <p style={{ textAlign: "center", marginTop: "20px", fontSize: "13px", color: "#999" }}>
          Não recebeu?{" "}
          <a href="/designer/login" style={{ color: "#00C9A7", fontWeight: "600", textDecoration: "none" }}>
            Tentar novamente
          </a>
        </p>
      </div>
